from text_crypto import encrypt_text, decrypt_text
from debug import debug_encrypt_text
from attack import brute_force

from text_crypto_key import encrypt_text as encrypt_text_fixed
from text_crypto_key import decrypt_text as decrypt_text_fixed
from debug_key import debug_encrypt_text as debug_encrypt_text_fixed


def main():
    key = 0b0100101011110101
    cifrado = []
    texto_original = ""

    while True:
        print("\n=== MINI AES ===")
        print("1 - Criptografar texto (chave manual)")
        print("2 - Mostrar criptografado")
        print("3 - Descriptografar (chave manual)")
        print("4 - Mostrar descriptografado")
        print("5 - Mostrar tudo")
        print("6 - Debug (chave manual)")
        print("7 - Quebrar chave (força bruta)")
        print("8 - Criptografar (chave fixa)")        # NOVO
        print("9 - Debug (chave fixa)")               # NOVO
        print("0 - Sair")

        opcao = input("Escolha: ")

        match opcao:
            case "1":
                texto_original = input("Digite o texto: ")
                cifrado = encrypt_text(texto_original, key)
                print("✅ Criptografado!")

            case "2":
                print("Cifrado:", cifrado if cifrado else "Nada ainda")

            case "3":
                if cifrado:
                    texto_decifrado = decrypt_text(cifrado, key)
                    print("✅ Descriptografado!")
                else:
                    print("Nada para descriptografar")

            case "4":
                if cifrado:
                    print("Texto:", decrypt_text(cifrado, key))
                else:
                    print("Nada ainda")

            case "5":
                if cifrado:
                    print("Original:", texto_original)
                    print("Cifrado:", cifrado)
                    print("Decifrado:", decrypt_text(cifrado, key))
                else:
                    print("Nada ainda")

            case "6":
                if texto_original:
                    debug_encrypt_text(texto_original, key)
                else:
                    print("Digite um texto primeiro!")

            case "7":
                if cifrado:
                    known = input("Digite o texto original (ex: oi): ")
                    brute_force(cifrado[0], known)
                else:
                    print("Nada criptografado ainda")

            case "8":
                texto_original = input("Digite o texto: ")
                cifrado = encrypt_text_fixed(texto_original)
                print("✅ Criptografado com chave fixa!")

            case "9":
                if texto_original:
                    debug_encrypt_text_fixed(texto_original)
                else:
                    print("Digite um texto primeiro!")

            case "0":
                print("Saindo...")
                break

            case _:
                print("Opção inválida!")


if __name__ == "__main__":
    main()